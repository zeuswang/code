...
{
	std::string key1 = key;
	sogou_string::ToLowerCase(key1);
	std::map<std::string,std::string>::const_iterator entry = content_map_.find(key1);
...
std::string HttpProtocolAdapter::GetHeaderParameter(const std::string& key)
{
	std::string key1 = key;
	sogou_string::ToLowerCase(key1);
	std::map<std::string,std::string>::const_iterator entry = header_map_.find(key1);

	if (entry != header_map_.end()) {
		return entry->second;
	} else {
		return "";
	}
...
int HttpProtocolAdapter::GetRequest()
{
	...
	if (!(conn_->GetFlag()&READABLE_SOCKET)){
		ret =-1;
		_err("HttpProtocolAdapter::GetRequest [GetFlag() is not READABLE_SOCKET]\n");
		goto OUT;
	}
	...
	if (read_http_header_timeout(httpheader_, &timeout) == 0 )
	{
		...
		{		
			...
			_err("HttpProtocolAdapter::GetRequest [Package Invalid (%s)]\n",httpheader_.c_str());
			//return 0;
			ret = -1;
		}		
	}else {
		_err("HttpProtocolAdapter::GetRequest [read_http_header_timeout() error]\n");
		ret=-1;
	}
OUT:
	if (ret ==0){
...
int HttpProtocolAdapter::GetResult()
{
	...
	if (!(conn_->GetFlag()&READABLE_SOCKET)){
		ret =-1;
		_err("[GetFlag() is not READABLE_SOCKET]\n");
		goto OUT;
	}
	if (read_http_header_timeout(httpheader_, &timeout) == 0 )
	{
		...
		{		
			...
			_err("[Package Invalid (%s)]\n",httpheader_.c_str());
			//return 0;
			ret = -1;
		}		
	}else {
		_err("[read_http_header_timeout() error]\n");
		ret=-1;
	}
OUT:
	if (ret ==0){
...
{
	fprintf(stderr,"HttpProtocolAdapter::begin\nheader=%s\n",header.c_str());
	char uri[2048];
...
int HttpProtocolAdapter::read_http_header_timeout(std::string &http, timeval *timeout)
{
	...
	while(1)
	{
		...
		if((n = conn_->ReadTimeoutOnce(buf,READ_BUF_SIZE,timeout))<0){
			_err("read_http_header_timeout conn_->ReadTimeout error\n");
			return -1;
		}
		buf[n] = '\0';
		_buffer += buf;
